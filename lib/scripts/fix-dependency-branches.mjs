...
/*
 * Esse script corrige as branches das dependências mapping para apontarem para a branch especificada
...
try {
  ...
  if (status.stdout) {
    await $`git add ${paths}`;
    await $`git commit -m "build(pipeline): Atualização das branches de dependências mapping para branch de destino [skip ci]"`;
